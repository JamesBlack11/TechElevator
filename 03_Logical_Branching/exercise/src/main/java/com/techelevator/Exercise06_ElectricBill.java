...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed, boolean hasRenewableEnergy) {
        ...
        if (unitsUsed > EXCESS_UNITS_LIMIT && hasRenewableEnergy) {
            return ((unitsUsed - EXCESS_UNITS_LIMIT) * EXCESS_RATE + (EXCESS_UNITS_LIMIT * BASE_RATE)) * DISCOUNT_FACTOR;
        }
        if (unitsUsed > EXCESS_UNITS_LIMIT) {
            return (unitsUsed - EXCESS_UNITS_LIMIT) * EXCESS_RATE + (EXCESS_UNITS_LIMIT * BASE_RATE);
        }
...
//        if (unitsUsed <= EXCESS_UNITS_LIMIT && hasRenewableEnergy && unitsReturned) {
//            return unitsUsed * BASE_RATE * (DISCOUNT_FACTOR);
//
//        }
//        if (unitsUsed <= EXCESS_UNITS_LIMIT) {
