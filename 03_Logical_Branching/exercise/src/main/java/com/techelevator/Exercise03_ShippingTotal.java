...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        if (weightPounds <= 40) {
            return weightPounds * 0.50;
        }
        if (weightPounds > 40) {
            return (weightPounds - 40) * 0.75 + 20;
        }
        return 0;
    }
...
//            return (weightPounds - (weightPounds * 0.1)) * 0.50;
//        }
//        if (weightPounds > 40 && hasDiscount) {
//            return ((weightPounds - 40) * 0.75 + 20) - (weightPounds * 0.1) / 2;
//        }
        double shippingCost = 0.0;
        if (weightPounds <= 40) {
            shippingCost = weightPounds * 0.50;
        } else {
            shippingCost = (weightPounds - 40) * 0.75 + 20;
        }
        if (hasDiscount) {
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        ...
        if (hasDiscount) {
            shippingCost = shippingCost * (1 - discountPercentage);
        }
        return shippingCost;
    }
    }
